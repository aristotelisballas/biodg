...
from ECG.models import networks
from ECG.models.biodgmodels import BioDGResNet18
from ECG.lib.misc import (
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        if self.hparams['resnet18']:
            self.classifier = nn.Sequential(nn.Linear(512, 24), nn.Sigmoid())
        else:
            self.classifier = nn.Sequential(nn.Linear(256*625, 24), nn.Sigmoid())
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        loss = criterion(self.predict(all_x), all_y.float())
...
#         all_x = torch.cat([x for x, y in minibatches])
#         all_y = torch.cat([y for x, y in minibatches])
#         loss = F.cross_entropy(self.predict(all_x), all_y.float())
#
#         self.optimizer.zero_grad()
#         loss.backward()
#         self.optimizer.step()
...
class AbstractDANN(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains,
        ...
        self.classifier = networks.Classifier(
            ...
            num_classes,
            self.hparams['nonlinear_classifier'])
        self.discriminator = networks.MLP(256*625,
            num_domains, self.hparams)
        self.class_embeddings = nn.Embedding(num_classes, 256*625)
...
class AbstractMMD(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for i in range(nmb):
            ...
            objective += criterion(classifs[i], targets[i].float())
...
class SagNet(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        loss_c = criterion(self.forward_c(all_x), all_y.float())
        loss_c.backward()
...
class RSC(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # Equation (5): update
        # loss = F.cross_entropy(all_p_muted_again, all_y.float())
        criterion = nn.BCELoss()
        loss = criterion(all_p_muted_again, all_y)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
